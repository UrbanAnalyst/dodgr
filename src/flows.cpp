...
//' @noRd
// [[Rcpp::export]]
Rcpp::NumericVector rcpp_flows_si_np (const Rcpp::DataFrame graph,
        const Rcpp::DataFrame vert_map_in,
        Rcpp::IntegerVector fromi,
        Rcpp::IntegerVector toi_in,
        Rcpp::NumericVector kvec,
        ...
        const std::string heap_type)
{
    ...
    // kvec can have multiple vectors of k-values, each equal in length to
    // the number of 'from' points. The output is then a single vector of
    // 'nedges' wrapped 'nk' times.
    const R_xlen_t nfrom_r = dens_from.size ();
    const R_xlen_t nk = kvec.size () / nfrom_r;
    const size_t nk_st = static_cast <size_t> (nk);
    ...
    for (size_t i = 0; i < nfrom; i++)
    {
        ...
        std::vector <double> w (nverts, INFINITE_DOUBLE);
        std::vector <double> d (nverts, INFINITE_DOUBLE);
        std::vector <int> prev (nverts);

        unsigned int from_i = static_cast <unsigned int> (fromi [i]);
        d [from_i] = w [from_i] = 0.0;

        // translate k-value to distance limit based on tol
        // exp(-d / k) = tol -> d = -k * log (tol)
        // const double dlim = -kvec [i] * log (tol);
        // kvec holds nk vectors of different k-values, each of length
        // nedges.
        double dlim = 0.0;
        for (R_xlen_t k = 0; k < nk; k++)
            if (kvec [i_R + k * nfrom] > dlim)
                dlim = kvec [i_R + k * nfrom];
        dlim = -dlim * log (tol);
            ...
            if (from_i != toi [j]) // Exclude self-flows
            {
                if (d [toi [j]] < INFINITE_DOUBLE)
                {
                    ...
                    for (size_t k = 0; k < nk_st; k++)
                    {
                        const R_xlen_t k_R = static_cast <R_xlen_t> (k);
                        exp_jk [k] = dens_to [j_R] *
                            exp (-d [toi [j]] / kvec [i_R + k_R * nfrom]);
                        flow_ijk [k] = dens_from [i_R] * exp_jk [k];
                        expsum [k] += exp_jk [k];
                    }
                    ...
                    while (target < INFINITE_INT)
                    {
                        ...
                        if (norm_sums)
                        {
                            path_len = 0;
                            size_t target_t = toi [j];
                            size_t from_t = static_cast <size_t> (fromi [i]);
                            while (target_t < INFINITE_INT)
                            {
                                path_len++;
                                target_t = prev [target_t];
                        ...
                        if (prev [stt] >= 0 && prev [stt] < INFINITE_INT)
                        {
                            ...
                            for (size_t k = 0; k < nk_st; k++)
                            {
                                flows_i [index + k * nedges] += flow_ijk [k] / path_len;
                            }
                        }
