...
DGraphInt::DGraphInt(int n)
{
    nVertices = n;
    
    vertices = new DGraphVertexInt[n];
    initVertices();
}
...
void DGraphInt::clear()
{
    DGraphEdgeInt *edge, *nextEdge;
    for(int i = 0; i < nVertices; i++) {
        edge = vertices[i].outHead;
...
void DGraphInt::addNewEdge(int source, int target, int dist, int wt)
{
    DGraphEdgeInt *newEdge = new DGraphEdgeInt;
    newEdge->source = source;
    newEdge->target = target;
    newEdge->dist = dist;
    newEdge->wt = wt;
    ...
    DGraphVertexInt *vertex = &vertices[source];
    if(vertex->outTail) {
        vertex->outTail->nextOut = newEdge;
    }
    else {
...
bool DGraphInt::edgeExists(int v, int w) const
{
    /* Scan all existing edges from v to determine whether an edge to w exists.
     */
    const DGraphEdgeInt *edge = vertices[v].outHead;
    while(edge) {
        if(edge->target == w) return true;
        edge = edge->nextOut;
    }
...
bool DGraphInt::reachable (int s) const
{
    int *stack = new int [nVertices];
    int tos = 0;
