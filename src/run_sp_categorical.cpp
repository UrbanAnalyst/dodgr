        ...
        const std::string& heap_type)
{
    ...
    if (nfrom != nto)
    {
        Rcpp::stop ("Pairwise categorical dists require equal numbers of 'from' and 'to' points.");
    }
    ...
    // Create parallel worker
    size_t chunk_size = run_sp::get_chunk_size (nfrom);
    OnePairedCategoricalDist one_dist (RcppParallel::RVector <int> (fromi),
            toi, edge_type, nverts, vx, vy,
