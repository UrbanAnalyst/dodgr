...
test_that ("points to graph", {
    ...
    expect_message (
        index1 <- match_pts_to_graph (net, pts),
        paste0 (
            "First argument to match_pts_to_graph should ",
    ...
    colnames (pts) <- NULL
    expect_message (
        index3 <- match_pts_to_graph (v, pts),
    ...
    expect_error (
        index4 <- match_pts_to_graph (v, list (pts)),
        "xy must be a matrix or data.frame"
    )
    expect_error (
        index4 <- match_pts_to_graph (v, pts),
