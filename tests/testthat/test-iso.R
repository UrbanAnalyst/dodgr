...
test_that ("isodists", {
    ...
    expect_named (d, c ("from", "dlim", "id", "x", "y"))
    expect_gt (nrow (d), 0)
    # some points may give no iso countours, so the following 2 may
    # not always be equal:
    expect_gte ((npts - length (unique (d$from))), 0L)
    expect_lte (length (unique (d$dlim)), length (dlim))

    expect_silent (dc <- dodgr_isodists (net, from = from, dlim = dlim, concavity = 0.1))
    expect_gt (nrow (dc), nrow (d)) # More detail in contours
    expect_named (dc, names (d))
    ...
    expect_gt (nrow (v), nrow (dc))
    expect_named (v, names (dc))
...
test_that ("turn penalty", {
    ...
    expect_identical (nrow (net), nrow (net0))
    d <- dodgr_isodists (net, from = from, dlim)
    expect_false (nrow (d) == nrow (d0))
    expect_false (any (grepl ("_start", d$from, fixed = TRUE)))
    expect_false (any (grepl ("_start", d0$from, fixed = TRUE)))
})
...
test_that ("isochrones", {
    ...
    expect_identical (ncol (x), 5L)
    expect_named (x, c ("from", "tlim", "id", "x", "y"))
    expect_gt (nrow (x), 0)
    # some points may give no iso countours, so the following 2 may
    # not always be equal:
    expect_gte ((npts - length (unique (x$from))), 0L)
    expect_lte (length (unique (x$tlim)), length (tlim))
})
