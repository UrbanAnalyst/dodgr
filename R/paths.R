...
#' Expand a contracted path back on to the full, uncontracted graph.
#'
#' A path calculated with \link{dodgr_paths} on a graph contracted with
#' \link{dodgr_contract_graph} traces only the intervening junction vertices,
#' skipping any intermediate (straight-line) vertices removed during
#' contraction. This function expands such a path back into the full sequence
#' of edges of the original, uncontracted graph.
#'
#' @param path_c A single path along a contracted graph, generally
#' generated by calling \link{dodgr_paths} on a graph previously contracted
#' with \link{dodgr_contract_graph}. May be given either as a character
#' vector of vertex IDs (as returned by \link{dodgr_paths} with `vertices =
#' TRUE`), or as an integer vector of row indices into `graph_c`.
#' @param graph The full, uncontracted graph from which `graph_c` was
#' generated with \link{dodgr_contract_graph}.
#' @param graph_c The contracted graph on which `path_c` was calculated.
...
#'
#' @return A `data.frame` of the rows of `graph` corresponding to the full,
#' uncontracted sequence of edges traced by `path_c`, ordered from start to
#' end of the path.
#' @family distances
#' @export
#' @examples
...
#' path_c <- dodgr_paths (graph_c, from = from, to = to) [[1]] [[1]]
#' path <- dodgr_paths_expand (path_c, graph, graph_c)
dodgr_paths_expand <- function (path_c, graph, graph_c, edge_map = NULL) {
