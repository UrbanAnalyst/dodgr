...
182,420 edges, representing 97.4% of the graph. The total number of distinct
components is
```{r, eval = FALSE}
...
`graph`. This may make the spatial mapping of routing points less accurate that
...
distances between 100 random points (and which uses the `magrittr` pipe
operator, `%>%`):
```{r, eval = FALSE}
require (magrittr)
bb <- osmdata::getbb ("york uk")
...
3. trinomial and trinomial heaps [@Takaoka2000]; and
...
```{r, eval = FALSE}
compare_heaps (graph, nverts = 1000)
```
```{r, echo = FALSE}
