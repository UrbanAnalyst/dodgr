...
```{r}
nrow (graph); nrow (grc$graph); nrow (grc$graph) / nrow (graph)
```
...
a standardised minimal form used for routing with the `convert_graph()`
function:
```{r}
graph <- convert_graph (graph)$graph
grc <- convert_graph (grc$graph)$graph
names (grc)
```
The `convert_graph` function also returns a matrix of all vertices (`$xy`), with
the graph again in a list item named `$graph`.  The difference in routing
