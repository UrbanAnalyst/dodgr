...
kinds of networks, one of the primary motivation is routing through street
networks.  Accordingly, most of the applications illustrated here concern street
...
cn
```
Components are numbered in order of decreasing size, with the largest in this
case containing 182,420 edges, representing 97.4% of the graph.

Component numbers can be determined for any types of graph with the
`dodgr_components()` function. For example, the previous graph can be reduced to
a minmial (non-spatial) structure:
```{r, eval = FALSE}
...
data.frame (edge_id = 1:6,
            ...
            component = rep (1, 6))
```
The `component` column column can be used to select any component from a graph.
It is particularly useful to ensure routing calculations consider only connected
vertices, as described below (under Distance Matrices).

## Weighting Profiles

The dual-weights for street networks are obtained by multiplying the distance of
each segment by a weighting factor reflecting the type of highway.  Each type of
highway has its own weight defined by the named weighting profiles included with
the `dodgr` package:
```{r}
