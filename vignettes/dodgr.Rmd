...
If the edge weights corresponded to distances, and if thicker lines represented
shorter distances, such that the distance `D`$\to$`C` was shorter than `C`$\to$`D`,
then it would be much shorter to go from `D` to `A` than the other way around.

Graphs in `dodgr` are represented by simple flat `data.frame` objects, so the
graph of Fig. 1, presuming the edge weights to take values of 1 and 2, would be,
```{r, echo = FALSE}
...
how one gets there: the route a pedestrian might taken will generally differ
...
specified with two weights or distances: one quantifying the physical distance,
and a second weighted version reflecting the mode of transport. Walking one
kilometre along a busy highway is more arduous than one kilometre walked through
a park, and so the two are weighted differently.

`dodgr` calculates shortest paths using one set of weights (called "weights" or
anything else starting with "w"), but subsequently calculates the actual lengths
of them using a second set of weights (called "distances", or anything else
starting with "d"). Consider that the weights and distances of Fig. 2 are the
black and grey lines, respectively, with the latter all equal to one. In this
case, the graph and associated shortest distances are,
```{r, echo = FALSE}
graph <- data.frame (from = c ("A", "B", "B", "B", "C", "C", "D", "D"),
                     to = c ("B", "A", "C", "D", "B", "D", "C", "A"),
                     w = c (1, 2, 1, 3, 2, 1, 2, 1),
...
1 + 3 = 4. The shortest *weighted* path is A$\to$B$\to$C$\to$D, with a weighted
distance of 1 + 1 + 1 = 3. Thus `d(A,D) = 3` and not 2.

# Introduction to the package

Although the package was intentionally developed to be adaptible to any kinds of
...
1and calculated one million shortest-path distances, all in under 30 seconds.
