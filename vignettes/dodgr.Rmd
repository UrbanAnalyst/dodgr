...
message (paste0 ('Error in igraph::distances(igr, v = nodes, to = nodes, ',
                 'mode = "out") :\n   At vector.pmt:442 : cannot reserve space ',
                 'for vector, Out of memory'))
```
...
from graphs, thereby speeding up routing calculations.

```{r, fig.align="center", echo=FALSE, engine='tikz',fig.ext='svg', code=readLines("fig3.tex"), fig.cap="Figure 3: The vertex B is redundant for routing calculations from anywhere other than that point itself"}
```

In Fig. 3, the only way to get from vertex A to D or E is through C. The
intermediate vertex B is redundant for routing purposes and may simply be
removed, with directional edges inserted directly between A and C. Note that if
one of the edges between, say, B and C were removed, vertex B would no longer be
redundant.
